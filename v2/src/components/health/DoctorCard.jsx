  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {/* Specialty */}
        <p className="text-xs text-[#6B7280] mb-1.5">{doctor.specialty}</p>

        {/* Status + Price + Cashback - Single line */}
        <div className="flex items-center gap-1.5 text-[10px] whitespace-nowrap overflow-hidden">
          {isOnline ? (
            <>
              <span className="text-[#00C853] font-500 flex items-center gap-0.5 flex-shrink-0">
                <span className="w-1 h-1 bg-[#00C853] rounded-full"></span>
                Онлайн
              </span>
              <span className="text-[#4A5268]">•</span>
              <span className="text-[#AAB3C5] flex-shrink-0">~{doctor.waitTime} мин</span>
            </>
          ) : (
          ...
          {/* Price + Cashback */}
          <span className="text-[#AAB3C5] font-500 flex-shrink-0">{doctor.price.toLocaleString()} ₸</span>
          <span className="text-[#00C853] font-600 flex-shrink-0">+{doctor.cashback.toLocaleString()} ₸</span>
        </div>
      </div>
